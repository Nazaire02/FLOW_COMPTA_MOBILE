...
import { getAllBanque } from '@/services/transactionService';
...
export default function Banque() {
    const [transBanques, setTransBanques] = useState<TransBanqueItem[]>([])
    async function getTransBanques() {
        try {
            const resp = await getAllBanque();
            setTransBanques(resp.data.data)
        } catch (error) {
            Alert.alert(
                "Erreur",
    ...
    const filteredData: TransBanqueItem[] = transBanques.filter(item => item.libelle.toLowerCase().includes(searchQuery.toLowerCase()));
